...
class MLS():
    ...
    def mlsd_2d_mapping(self, image_shape, mlsd, points_end, grid_x, grid_y):
        ...
        x, y = np.arange(image_shape[0], step=self.step), np.arange(image_shape[1], step=self.step)
        xi, yi = np.arange(image_shape[0], step=1), np.arange(image_shape[1], step=1)

        dxT = self.interp2(x, y, vx, xi, yi)
        dyT = self.interp2(x, y, vy, xi, yi)
        
        mapping = self.Tensor(1, 2, image_shape[0], image_shape[1])
        
        for i in range(image_shape[0]):
            for j in range(image_shape[1]):
                mapping[:, :, i, j] = self.Tensor([i + dxT[i, j], j + dyT[i, j]])
        return mapping

    def MLS_interp(self, image_shape, points_start, points_end):
        assert (points_start.shape[0] == points_end.shape[0])
        grid_x, grid_y = np.meshgrid(np.arange(image_shape[0], step=self.step), np.arange(image_shape[1], step=self.step))
        gv = np.column_stack((grid_y.flatten(), grid_x.flatten()))
        mlsd = self.get_precompute_points(points_start, gv)
        mapping = self.mlsd_2d_warp(image_shape, mlsd, points_end, grid_x, grid_y)
        return mapping
    ...
    def run_MLS_in_folder(self, root_folder):
        ...
        plt.imsave('%s/warp_AtoB.png' % root_folder, warp_AtoB)
